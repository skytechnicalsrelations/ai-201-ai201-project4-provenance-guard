...
**Calibration outcome:** thresholds and weights were asserted in code against the spec and matched exactly on the first pass (`0.6/0.4`, `0.35`/`0.70` — no drift). The first calibration run still failed the `formal-human` case, but not because of weights or thresholds — the generated type-token-ratio normalization (`0.40–0.80`) assumed longer text than the ~40-55 word reference inputs actually contain; real TTR for short text clustered at `0.86–0.90`, saturating the metric to `1.0` for every input and contributing nothing. Re-centering to `0.85–0.95` (per-metric bounds, not weights/thresholds) fixed all four bands, including the critical `formal-human -> uncertain` case at confidence `0.669`. See README's "Confidence Scoring → Calibration" section for the full results table.
